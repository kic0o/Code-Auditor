...
const REVIEW_UPLOAD_CONFIG = {
  code: {
    label: 'Revision de codigo',
    helper: 'Arrastra archivos de codigo o un ZIP del proyecto',
    accept: '.js,.jsx,.ts,.tsx,.py,.java,.cs,.cpp,.c,.go,.php,.rb,.rs,.swift,.kt,.sql,.json,.yaml,.yml,.xml,.zip',
    allowedExtensions: ['js', 'jsx', 'ts', 'tsx', 'py', 'java', 'cs', 'cpp', 'c', 'go', 'php', 'rb', 'rs', 'swift', 'kt', 'sql', 'json', 'yaml', 'yml', 'xml', 'zip'],
  },
  documentation: {
    label: 'Revision de documentacion',
...
const App = () => {
  ...
  const [uploadReviewType, setUploadReviewType] = useState('documentation');
...
const categorySelectionCounts = useMemo(() => (
  ...
  const processDroppedFiles = (incomingFiles, reviewType) => {
    ...
    if (!validFiles.length) {
      setError(`No se detectaron archivos validos para ${label.toLowerCase()}.`);
      return;
    }

    if (validFiles.length !== files.length) {
      setError(`Algunos archivos no se agregaron porque no corresponden a ${label.toLowerCase()}.`);
    } else {
      setError(null);
    }
...
<div className="lg:col-span-4 space-y-6">
  ...
  <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
    <div className="flex items-center gap-3 mb-4">
      ...
      <h2 className="text-sm font-bold text-slate-800">Carga de Archivos</h2>
    </div>

    <div className="grid grid-cols-2 gap-2 mb-4 rounded-xl bg-slate-100 p-1">
      {Object.entries(REVIEW_UPLOAD_CONFIG).map(([key, config]) => (
        <button
          key={key}
          type="button"
          onClick={() => setUploadReviewType(key)}
          className={`rounded-lg px-3 py-2 text-[11px] font-bold transition-all ${
            uploadReviewType === key
    ...
    <input 
      ...
      multiple 
      accept={REVIEW_UPLOAD_CONFIG[uploadReviewType].accept}
      className="hidden" 
    />
      ...
      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter text-center">
        {isDraggingFiles
          ...
          : uploadedDocs.length > 0
            ...
            : REVIEW_UPLOAD_CONFIG[uploadReviewType].helper}
      </p>
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wide text-slate-400">
                      {REVIEW_UPLOAD_CONFIG[doc.reviewType]?.label || 'Archivo'}
                    </span>
                  </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden h-[750px] flex flex-col">
        ...
        <div className="bg-[#0f172a] p-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h3 className="text-sm font-bold tracking-wide">Matriz de Analisis</h3>
              <p className="text-[10px] font-medium text-slate-400">
                Marca cada archivo en una o varias categorias segun el analisis que necesites.
              </p>
            </div>
          </div>
                    ...
                    {ANALYSIS_CATEGORIES.map(category => {
                      ...
                      return (
                        <button
                          ...
                          title={`${category.label}: ${isSelected ? 'seleccionado' : 'no seleccionado'}`}
                        >
