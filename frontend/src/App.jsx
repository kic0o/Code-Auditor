...
const App = () => {
    ...
    <nav className="bg-[#0f172a] px-8 py-3 flex justify-between items-center sticky top-0 z-20 shadow-lg">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-baseline gap-2">
          <h1 className="text-2xl font-bold text-white tracking-tight">AI Code Auditor</h1>
          <span className="text-[12px] font-bold text-slate-400 bg-slate-800 px-2 py-0.5 rounded">V1.0</span>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 px-4 py-1.5 bg-blue-900/30 border border-blue-500/30 rounded-full">
          ...
          <span className="text-[13px] font-bold text-blue-300 tracking-wide">Gemini 1.5 Flash · Conectado</span>
        </div>
        <div className="w-9 h-9 bg-slate-800 rounded-lg flex items-center justify-center text-slate-300 font-bold text-sm border border-slate-700">ES</div>
      </div>
    </nav>
  );
  ...
  const Footer = () => (
    <footer className="bg-white border-t border-slate-200 px-8 py-4 mt-auto">
      <div className="max-w-7xl mx-auto flex justify-between items-center text-[12px] font-bold text-slate-400 uppercase tracking-widest">
    ...
    <div className="flex justify-center items-center flex-1">
      <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">Conecta tu repositorio</h2>
          <p className="text-slate-400 text-base mt-1">Ingresa la URL de GitHub para comenzar</p>
        </div>
        <section className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <div className="space-y-4">
            <div>
              <label className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-2 block">URL DE GITHUB</label>
              <div className="relative">
                <input type="text" value={repoUrl} onChange={e => setRepoUrl(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && handleConnect()}
                  placeholder="https://github.com/..."
                  className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-base font-medium focus:ring-2 focus:ring-blue-500/20 outline-none transition-all" />
                <Search className="absolute left-3.5 top-3.5 text-slate-400" size={16} />
              </div>
            </div>
            {error && <p className="text-sm text-red-500 font-bold px-3 py-2 bg-red-50 rounded-xl border border-red-100">{error}</p>}
            <button onClick={handleConnect} disabled={loadingFiles || !repoUrl}
              className={`w-full py-3.5 text-white text-base font-bold rounded-xl transition-all flex justify-center items-center gap-2 shadow-md ${loadingFiles || !repoUrl ? 'bg-slate-300 cursor-not-allowed shadow-none' : 'bg-[#0f172a] hover:bg-slate-800 shadow-slate-200 active:scale-95'}`}>
      ...
      <div className="lg:col-span-4 space-y-6">
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-base font-bold text-slate-800">Repositorio Conectado</h2>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            <span className="text-sm font-mono text-slate-600 truncate">{repoUrl}</span>
          </div>
          <button onClick={handleReset} className="mt-3 text-[12px] font-bold text-slate-400 hover:text-blue-600 transition-colors uppercase tracking-wider">← Cambiar repositorio</button>
        </section>
 
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-6">
            <div className="bg-slate-900 p-2 rounded-lg text-white"><Zap size={18} /></div>
            <h2 className="text-base font-bold text-slate-800">Resumen del Repo</h2>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-3xl font-bold text-blue-600">{selectedFilesCount}<span className="text-slate-300 text-2xl">/{repoFiles.length}</span></p>
              <p className="text-[12px] font-bold text-slate-400 uppercase tracking-tighter leading-tight">Archivos En Matriz</p>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-3xl font-bold text-slate-800">{totalSelectionsCount}</p>
              <p className="text-[12px] font-bold text-slate-400 uppercase tracking-tighter leading-tight">Asignaciones Totales</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-base font-bold text-slate-800">Carga de Documentos</h2>
          </div>
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center bg-slate-50 hover:bg-blue-50/50 hover:border-blue-300 transition-all cursor-pointer group"
            ...
            <p className="text-[12px] font-bold text-slate-500 uppercase tracking-tighter text-center">
              ...
              {uploadedDocs.map((doc, i) => (
                <div key={i} className={`flex items-center justify-between p-3 rounded-xl border ${doc.status === 'processed' ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-[13px] font-medium truncate">{doc.name}</span>
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden h-[750px] flex flex-col">
          <div className="bg-[#0f172a] p-5 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
            <div className="flex items-center gap-3 text-white">
              ...
              <h3 className="text-base font-bold tracking-wide">Matriz de Análisis</h3>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {ANALYSIS_CATEGORIES.map(c => (
                <div key={c.id} className="rounded-xl border border-slate-700 bg-slate-900/70 px-3 py-2">
                  <p className="text-[12px] font-bold uppercase tracking-wide text-slate-400">{c.shortLabel}</p>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-sm font-bold text-white">{categorySelectionCounts[c.id]}</span>
                    <button onClick={() => selectAllForCategory(c.id)} className="text-[11px] font-bold uppercase text-blue-300 hover:text-white">Todo</button>
                    <button onClick={() => clearCategory(c.id)} className="text-[11px] font-bold uppercase text-slate-400 hover:text-white">Limpiar</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-4 border-b border-slate-100 bg-slate-50/50">
            <div className="relative">
              <input type="text" placeholder="Filtrar archivos..." className="w-full bg-white border border-slate-200 rounded-lg pl-10 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20" />
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {repoFiles.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 opacity-50">
                <Folder size={48} className="mb-2" /><p className="text-sm font-medium italic">Sin archivos</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-[minmax(0,2.4fr)_repeat(4,minmax(88px,1fr))_110px] gap-2 px-3 text-[12px] font-black uppercase tracking-[0.18em] text-slate-400">
                  <span>Archivo</span>
                  ...
                  return (
                    <div key={idx} className={`grid grid-cols-[minmax(0,2.4fr)_repeat(4,minmax(88px,1fr))_110px] gap-2 rounded-2xl border p-3 transition-all ${inAny ? 'bg-blue-50/60 border-blue-200' : 'bg-white border-slate-100'}`}>
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="bg-slate-100 p-1.5 rounded text-[12px] font-bold text-slate-500 uppercase">{file.extension.replace('.', '')}</div>
                        <span className="truncate text-sm font-medium text-slate-700">{file.path}</span>
                      </div>
                      {ANALYSIS_CATEGORIES.map(c => {
                        ...
                        return (
                          <button key={c.id} onClick={() => toggleCategorySelection(file.path, c.id)}
                            ...
                            {isSel ? <Check size={14} /> : <span className="text-[12px] font-bold">+</span>}
                          </button>
                        );
                      })}
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => selectFileForAllCategories(file.path)} className="rounded-lg border border-slate-200 px-2 py-1 text-[12px] font-bold text-slate-500 hover:border-blue-300 hover:text-blue-600">Todas</button>
                        <button onClick={() => clearFileFromAllCategories(file.path)} className="rounded-lg border border-slate-200 px-2 py-1 text-[12px] font-bold text-slate-500 hover:border-red-300 hover:text-red-500">Limpiar</button>
                      </div>
                    </div>
                  );
                })}
          ...
          {error && <p className="text-sm text-red-500 font-bold px-6 py-2 bg-red-50 border-t border-red-100">{error}</p>}
          <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
            ...
            <button onClick={startAnalysis} disabled={selectedFilesCount === 0}
              className={`px-8 py-3.5 rounded-xl font-bold text-sm flex items-center gap-3 transition-all shadow-lg ${selectedFilesCount > 0 ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200' : 'bg-slate-300 text-slate-500 cursor-not-allowed shadow-none'}`}>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="bg-[#0f172a] px-8 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-0.5">Evalúa cada hallazgo y decide si aceptas los cambios sugeridos</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 divide-x divide-slate-100 border-t border-slate-100">
            <div className="flex flex-col items-center justify-center py-6 px-4 md:col-span-1">
              ...
              <p className="text-[12px] font-black text-slate-400 uppercase tracking-widest mt-2">Score Global</p>
            </div>
            <div className="flex flex-col items-center justify-center py-6 px-4 gap-1">
              ...
              <span className="text-3xl font-black text-red-600">{reportStats.critical}</span>
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wide">Críticos</span>
            </div>
            <div className="flex flex-col items-center justify-center py-6 px-4 gap-1">
              <div className="w-10 h-10 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center mb-1">
                <AlertCircle size={18} className="text-amber-500" />
              </div>
              <span className="text-3xl font-black text-amber-600">{reportStats.warning}</span>
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wide">Advertencias</span>
            </div>
            <div className="flex flex-col items-center justify-center py-6 px-4 gap-1">
              <div className="w-10 h-10 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center mb-1">
                <Info size={18} className="text-blue-500" />
              </div>
              <span className="text-3xl font-black text-blue-600">{reportStats.info}</span>
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wide">Informativos</span>
            </div>
            <div className="flex flex-col items-center justify-center py-6 px-4 gap-1">
              <div className="w-10 h-10 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center mb-1">
                <FileText size={18} className="text-slate-500" />
              </div>
              <span className="text-3xl font-black text-slate-700">{reportStats.affectedFiles}</span>
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wide">Afectados</span>
              <div className="w-full mt-1 px-2">
                ...
                <p className="text-[11px] text-slate-400 text-center mt-0.5">{reportStats.affectedPct}% del total</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="lg:col-span-4">
            <div className="bg-transparent space-y-3 sticky top-24">
              <h3 className="text-[13px] font-black text-slate-400 uppercase tracking-[0.2em] px-1 mb-4">Submenú de Revisión</h3>
                  ...
                  <button key={category.id} type="button" onClick={() => goToCategory(index)}
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">Aspecto {index + 1}</p>
                          <h4 className={`text-base font-bold leading-tight ${isActive ? 'text-slate-800' : 'text-slate-600'}`}>{category.label}</h4>
                        </div>
                      </div>
                      {isDone ? (
                        <span className="text-[11px] font-black uppercase px-2 py-1 rounded-lg bg-green-100 text-green-700 flex-shrink-0">✓ Listo</span>
                      ) : isActive ? (
                        <span className="text-[11px] font-black uppercase px-2 py-1 rounded-lg bg-blue-100 text-blue-600 flex-shrink-0">Actual</span>
                      ) : null}
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      {stats.critical > 0 && (
                        <span className="flex items-center gap-1 text-[12px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-lg">
                          <ShieldAlert size={9} /> {stats.critical}
                        </span>
                      )}
                      {stats.warning > 0 && (
                        <span className="flex items-center gap-1 text-[12px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg">
                          <AlertCircle size={9} /> {stats.warning}
                        </span>
                      )}
                      {stats.info > 0 && (
                        <span className="flex items-center gap-1 text-[12px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg">
                          <Info size={9} /> {stats.info}
                        </span>
                      )}
                      {stats.total === 0 && <span className="text-[12px] font-bold text-slate-400">Sin hallazgos</span>}
                    </div>
              ...
              {!reviewCompleted ? (
                <>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                      <p className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-400">Revisión Actual</p>
                      <h3 className="text-3xl font-black text-slate-900 tracking-tight">{activeCategory.label}</h3>
                      <p className="text-base text-slate-400">Evalúa cada hallazgo y decide si aceptas la corrección.</p>
                    </div>
                    <div className="flex gap-3 flex-shrink-0">
                      <div className="rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-center">
                        <p className="text-[11px] font-black uppercase tracking-wide text-red-400">Críticos</p>
                      ...
                      <div className="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-center">
                        <p className="text-[11px] font-black uppercase tracking-wide text-amber-400">Alertas</p>
                      ...
                      <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-center">
                        <p className="text-[11px] font-black uppercase tracking-wide text-blue-400">Info</p>
                  ...
                  <div className="flex flex-wrap gap-3 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] font-black text-slate-400 uppercase">Severidad:</span>
                      <select value={filterSeverity} onChange={e => setFilterSeverity(e.target.value)}
                        className="text-sm font-bold bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500/20">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] font-black text-slate-400 uppercase">Tipo:</span>
                      <select value={filterType} onChange={e => setFilterType(e.target.value)}
                        className="text-sm font-bold bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500/20">
                        {uniqueTypes.map(t => <option key={t} value={t}>{t === 'all' ? 'Todos los tipos' : t}</option>)}
                      </select>
                    </div>
                    <span className="text-[12px] text-slate-400 ml-auto">{currentCategoryFindings.length} hallazgo{currentCategoryFindings.length !== 1 ? 's' : ''}</span>
                  </div>
                          ...
                          <div key={findingId} className={`rounded-2xl border overflow-hidden shadow-sm transition-all ${
                            ...
                            <div className={`px-5 py-2 flex items-center justify-between ${
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-[12px] font-black uppercase tracking-widest ${isC ? 'text-red-600' : isW ? 'text-amber-600' : 'text-blue-600'}`}>
                                  {isC ? 'Crítico' : isW ? 'Advertencia' : 'Informativo'}
                                </span>
                                {decision && (
                                  <span className={`text-[11px] font-black uppercase px-2 py-0.5 rounded-full ml-2 ${decision === 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                                    {decision === 'accepted' ? '✓ Aceptado' : '✗ Rechazado'}
                                  </span>
                                )}
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-[12px] font-bold bg-white/70 px-2 py-0.5 rounded-lg text-slate-600 font-mono">{finding.file_path}</span>
                                {finding.line && <span className="text-[12px] font-bold bg-white/70 px-2 py-0.5 rounded-lg text-slate-500">Línea {finding.line}</span>}
                                <span className={`text-[12px] font-bold px-2 py-0.5 rounded-lg ${isC ? 'bg-red-100 text-red-700' : isW ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>{finding.type}</span>
                              </div>
                            </div>
 
                            {/* Cuerpo */}
                            <div className="bg-white p-5 space-y-3">
                              <h4 className="text-base font-bold text-slate-800 leading-snug">{finding.title}</h4>
                              <p className="text-base text-slate-500 leading-relaxed">{finding.description}</p>
                              <div className="flex items-start gap-2 bg-slate-50 rounded-xl p-3 border border-slate-100">
                                <span className="text-base">💡</span>
                                <div>
                                  <span className="text-[12px] font-black uppercase tracking-wide text-slate-400 block mb-0.5">Sugerencia</span>
                                  <span className="text-base text-slate-600">{finding.recommendation}</span>
                                </div>
                              </div>
                              ...
                              {(finding.original_code || finding.secure_code) && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-1.5 text-[12px] font-black text-red-400 uppercase tracking-widest">
                                      <X size={10} /> Código Original
                                    </div>
                                    <pre className="bg-red-50/40 border border-red-100 p-4 rounded-xl text-[13px] font-mono text-red-800 overflow-x-auto whitespace-pre-wrap">
                                      <code>{finding.original_code || '# No hay código'}</code>
                                    </pre>
                                  </div>
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-1.5 text-[12px] font-black text-green-500 uppercase tracking-widest">
                                      <Check size={10} /> Código Corregido
                                    </div>
                                    <pre className="bg-green-50 border border-green-100 p-4 rounded-xl text-[13px] font-mono text-green-800 overflow-x-auto whitespace-pre-wrap shadow-inner">
                              ...
                              <div className="flex gap-3 pt-3 border-t border-slate-100">
                                <button onClick={() => toggleFindingDecision(findingId, 'rejected')}
                                  className={`flex-1 py-2.5 rounded-xl font-bold text-sm flex justify-center items-center gap-2 transition-all ${
                  ...
                  <div className="flex justify-between items-center border-t border-slate-100 pt-5">
                    <button onClick={handleReset} className="flex items-center gap-2 text-slate-400 font-bold hover:text-blue-600 transition-colors text-base">
                ...
                <div className="space-y-6">
                  <div className="text-center pb-4 border-b border-slate-100">
                    ...
                    <h3 className="text-3xl font-black text-slate-900 tracking-tight">Revisión Finalizada</h3>
                    <p className="text-slate-500 mt-1 text-base">Has evaluado todos los hallazgos propuestos por la IA.</p>
                  </div>
                      ...
                      return (
                        <div key={category.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-base font-bold text-slate-800">{category.label}</p>
                          </div>
                          <div className="flex gap-3 mb-3">
                            {stats.critical > 0 && <span className="text-[12px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-lg">{stats.critical} críticos</span>}
                            {stats.warning  > 0 && <span className="text-[12px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg">{stats.warning} alertas</span>}
                            {stats.info     > 0 && <span className="text-[12px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg">{stats.info} info</span>}
                            {stats.total === 0  && <span className="text-[12px] font-bold text-slate-400">Sin hallazgos</span>}
                          </div>
                          <div className="flex gap-3">
                            <div className="flex-1 text-center bg-green-50 rounded-xl py-2 border border-green-100">
                              <span className="text-2xl font-black text-green-600">{accepted}</span>
                              <p className="text-[11px] font-bold text-green-500 uppercase">Aceptados</p>
                            </div>
                            <div className="flex-1 text-center bg-red-50 rounded-xl py-2 border border-red-100">
                              <span className="text-2xl font-black text-red-500">{rejected}</span>
                              <p className="text-[11px] font-bold text-red-400 uppercase">Rechazados</p>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  <div className="grid grid-cols-3 gap-3 bg-slate-50 rounded-2xl border border-slate-200 p-4">
                    <div className="text-center">
                      ...
                      <p className="text-[12px] font-bold text-slate-400 uppercase mt-0.5">Total Aceptados</p>
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-[12px] font-bold text-slate-400 uppercase mt-0.5">Total Rechazados</p>
                    </div>
                    <div className="text-center">
                      <span className="text-3xl font-black text-slate-700">{analysisResult.findings?.length || 0}</span>
                      <p className="text-[12px] font-bold text-slate-400 uppercase mt-0.5">Total Hallazgos</p>
                    </div>
                  </div>
 
                  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 text-center">
                    <p className="text-base font-bold text-slate-600 mb-5">Estás a punto de inyectar las correcciones aceptadas a tu entorno virtual.</p>
                    <button onClick={enviarDecisionesAlBackend} disabled={isSendingToBackend}
                      className="w-full md:w-auto px-10 py-4 rounded-2xl font-black text-base flex items-center justify-center mx-auto gap-3 text-white bg-[#0f172a] hover:bg-slate-800 shadow-xl shadow-slate-300 transition-all active:scale-95">
                  ...
                  <button onClick={handleReset} className="block text-center w-full text-sm font-bold text-slate-400 hover:text-blue-600 transition-colors">
      ...
      <main className="flex-1 px-8 py-10 flex flex-col">
        ...
        {view === 'analyzing' && (
          <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in zoom-in duration-500">
            ...
            <div className="text-center">
              <h2 className="text-4xl font-black text-slate-800 tracking-tight italic">Analizando Matriz...</h2>
