...
export function useAnalysis() {
  ...
  const advanceToNextCategory = async (selectedFileMatrix, repoUrl, setView, setCurrentlyAnalyzingCategory) => {
    ...
    if (nextIndex < ANALYSIS_CATEGORIES.length) {
      ...
      if (!fetchedCategories.has(nextCatId)) {
        await fetchCategoryData(nextIndex, selectedFileMatrix, repoUrl, setView, setCurrentlyAnalyzingCategory);
      }
    }
  };
